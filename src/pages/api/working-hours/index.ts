...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getWorkingHours() {
    ...
    const response = await prisma.working_hours
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'working_hours'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createWorkingHours() {
    ...
    const data = await prisma.working_hours.create({
      data: body,
    });
