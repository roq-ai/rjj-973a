  ...
  const { data, error, isLoading, mutate } = useSWR<PerformanceEvaluationInterface>(
    ...
    () =>
      getPerformanceEvaluationById(id, {
        relations: ['user_performance_evaluation_user_idTouser', 'user_performance_evaluation_team_lead_idTouser'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Performance Evaluation User Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_performance_evaluation_user_idTouser?.id}`}>
                        {data?.user_performance_evaluation_user_idTouser?.email}
                      </Link>
                    }
                  />
                )}
